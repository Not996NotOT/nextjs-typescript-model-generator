...
class CodeController {
  ...
  convertJsonToOther() {
    ...
    this.convertToInterface(jObject);
    this.convertToClass(jObject);
  }

  convertToInterface(jObject) {
    let str = `export interface ${this.interfaceName} { \n`;
  ...
  convertToClass(jObject) {
    ...
    str += `export class ${this.className} extends AbsBaseDTO implements ${this.interfaceName} { \n`;
    let keys = Object.getOwnPropertyNames(jObject);
    if (keys.length > 0) {
      keys.forEach((key) => {
        str += `\t${key}:${this.getPropertyType(jObject[key])};\n`;
      ...
      <div className={styles.title}>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            controller.interfaceName = `I${e.target.value}`;
